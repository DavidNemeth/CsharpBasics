...
namespace MoreThreading
{
    class Program
    {
        ...
        static long[] portionResuts;
        static int portionSize;
        static void GenerateInts()
        {
            var randy = new Random(987);
            for (int i = 0; i < values.Length; i++)
            {
                values[i] = (byte)randy.Next(10);
            }
        }
        static void SumYourPortion(object portionNumber)
        {
            ...
            portionResuts[portionNumberAsInt] = sum;
        }
        static void Main(string[] args)
        {
            portionResuts = new long[Environment.ProcessorCount];
